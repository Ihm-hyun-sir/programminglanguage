 ...
 //  map<string,string> symbol_table;
   ...
   any visitExpression(BParser::ExpressionContext *ctx) override {
        if (ctx->ASSN()) { // assignment
			visit(ctx->name());
	//		cout << "expression var : " << current_var << " expression type " << symbol_table[current_var] << "\n";
			expression_var = current_var;
			expression_type = symbol_table[current_var];
		...
		{
			symbol_table[expression_var] = expr_type;
	//		cout << "Get Type " << expression_var << " to " << expr_type << "\n"; 
		}
   ...
   any visitAtom(BParser::AtomContext *ctx) override {
   	if(ctx->name())
	{
		visit(ctx->name());
		if(expr_type == "")
			expr_type = symbol_table[current_var];
		else
			compare_type = symbol_table[current_var];
   ...
   any visitDeclstmt(BParser::DeclstmtContext *ctx) override{
	   symbol_table.insert({ctx->name()->getText(),"F_NONE"});
	   for(int i = 1; i < ctx->AUTO().size() ; i++)
	   {
		   symbol_table.insert({ctx->name()->getText() + to_string(i-1)  ,"NONE"});
   ...
   any visitFuncdef(BParser::FuncdefContext *ctx) override{
   	   visit(ctx->statement());
	   symbol_table[ctx->name(0)->getText()] = return_type;
	   if(ctx->name(0)->getText() == "main" && symbol_table["main"] != "int")
	   {
		throw runtime_error("func type not matched expected : int get : "+return_type);
		symbol_table["main"] = "int";
	   }
	   if(ctx->name().size() > 1){
	   	for(int i = 1 ; i < ctx->name().size(); i++)
	   	{
			   symbol_table.erase(ctx->name(i)->getText());
	   	}
	   }
  ...
  any visitFuncinvocation(BParser::FuncinvocationContext *ctx) override {
	...
	for(int i = 0 ; i < ctx->expr().size() ; i++)
	{
		...
		compare_type = "";
		visit(ctx->expr(i));
		if(symbol_table[ctx->name()->getText()+to_string(i)] == "NONE")
			symbol_table[ctx->name()->getText() + to_string(i)] = expr_type;
		if(symbol_table[ctx->name()->getText() + to_string(i)] != expr_type)
		{			
			throw runtime_error("func parameter expr type not matched expected : "+symbol_table[ctx->name()->getText() + to_string(i)]+" get : "+expr_type);
		}	
	}
	expr_type = save_expr_type;
	compare_type = save_compare_type;
	if(expr_type == "")
                expr_type = symbol_table[function_name];
        else
                compare_type = symbol_table[function_name];
	return nullptr;	
  }
 };
...
public:
	...
	for(map<string,string>::iterator it = symbol_table.begin(); it != symbol_table.end(); it++)
	{
		if(it->second == "NONE")
		{
			throw runtime_error("Undefined Variable : "+it->first+"\n");
		}
	}
    	for (int i=0; i< ctx->children.size(); i++) {
      	    visit(ctx->children[i]);
    ...
    any visitFuncdef(BParser::FuncdefContext *ctx) override {
		...
		cout << symbol_table[ctx->name(0)->getText()] << " " << functionName << "(" ;
        // You can retrieve and visit the parameter list using ctx->name(i)
		for (int i=0; i < ctx->name().size()-1; i++) {
			cout << symbol_table[ctx->name(0)->getText()+to_string(i)] << " " <<ctx->name(i+1)->getText() << " ";
    ...
    any visitAutostmt(BParser::AutostmtContext *ctx) override {
		...
		string type = symbol_table[ctx->name(0)->getText()];	
	        string item;
		bool first_var = true;
		cout << type << " ";
		for (int i=1; i < ctx->children.size(); i++) {
		       ...
		       if(item == "," || item == ";")
			{
		       ...
		       else 
		       {
			       if(item == "auto")
				       continue;
			       if(type != symbol_table[item])
			       {
				       cout << ";";
				       type = symbol_table[item];
				       cout << "\n" << type << " " << item;
			       }
			       else
			       {
    ...
    any visitDeclstmt(BParser::DeclstmtContext *ctx) override {
		...
		cout << symbol_table[functionName] << " " << functionName << "(" ;
        
		// You can retrieve and visit the parameter type list
		for (int i=0; i < ctx->AUTO().size()-1; i++) {
			cout << symbol_table[ctx->name()->getText() + to_string(i)];
